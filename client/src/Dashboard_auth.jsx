...
function Dashboard_auth({ disabledButtons, handleAvailableClick, handleNotAvailableClick }){
    ...
    useEffect(() => {
        axios.get('barberbackend-digvijay1209s-projects.vercel.app/Dashboard_auth')
            .then(res => {
    ...
    const auth_c = (name, time, status, index) => {
        axios.post('barberbackend-digvijay1209s-projects.vercel.app/updated', { name, time, status })
            .then(result => {
                console.log(result);
    ...
    const reject_c = (name, time) => {
        axios.post('barberbackend-digvijay1209s-projects.vercel.app/rejected', { name, time })
            .then(result => {
                console.log(result);
                window.location.reload();
            }).catch(err => console.log(err));
